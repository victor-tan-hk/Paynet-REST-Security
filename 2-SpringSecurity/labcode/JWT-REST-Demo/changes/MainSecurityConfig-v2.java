...
public class MainSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication().passwordEncoder(passwordEncoder()).withUser(user1Name)
				.password(passwordEncoder().encode(user1Password)).roles(role1).and().withUser(user2Name)
				.password(passwordEncoder().encode(user2Password)).roles(role1).and().withUser(user3Name)
				.password(passwordEncoder().encode(user3Password)).roles(role2).and().withUser(user4Name)
				.password(passwordEncoder().encode(user4Password)).roles(role2);
